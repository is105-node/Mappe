...
func PrintTreasureUTF8(treasureString string) {
	k := []byte(treasureString)
	f:= bytes.Replace(k, []byte("\xe5"), []byte("\xc3\xa5"), -1)
	f = bytes.Replace(f, []byte("\xf8"), []byte("\xc3\xb8"), -1)
	f = bytes.Replace(f, []byte("\xe6"), []byte("\xc3\xa6"), -1)
	fmt.Printf("%s", f)
}